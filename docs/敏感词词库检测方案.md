...
* 不替换 LLM 审核层。
* **P1 不做灰名单（suspect）**：避免流式窗口高频强制 LLM 放大成本；政治类等高误伤分类留待 P2。

### 1.4 目标分层（避免误判进度）

| 阶段 | 实际目标 | 明确不是 |
|---|---|---|
| P1 | 补暴恐/色情/赌毒等**确定性**拦截；官方语料 0 误拦 block | 不等于备案达标（未覆盖 17 类、灰名单+审核模型未齐） |
| P2 | suspect + 政治类等高误伤分类；灰名单依赖 `AUDIT_MODEL` | 仍需单独合规 checklist / 评测报告 |
...
* **稳定 category 枚举**写在代码里（`terrorism` / `porn` / `gambling_drugs`），文件名只是数据源，禁止前端依赖可变文件名。
...
1. `AuditResult` / audit 路径增加 suspected  
2. 政治类等高误伤入 suspect  
3. 验证：LLM 判 0 放行 / 判 1 拦截 / LLM 挂掉放行  
...
| 归一化 | 全角/干扰字符变体命中同一词 |
| 自动机 | 多词命中；空文本；构建可加载 |
| 判定 | block 即决；allow 长词优先；开关关闭回退 |
| 热加载 | 目录 mtime 变化重建 |
| 回归 | blacklist.txt 既有用例全绿 |
| 误报基线 | 官方 assets + 模板文本 0 block 命中 |
