...
class AdminVisualLlmSettings(BaseModel):
    """平台预设视觉验收模型（C 级软验收）。GET 回 masked apikey；PUT 收明文（空/masked=保留旧值）。
...
class AdminSettings(BaseModel):
    ...
    visual_llm: AdminVisualLlmSettings | None = None
...
class AdminVisualLlmTestResp(BaseModel):
    """视觉模型连通测试：发一张探针图，tested_ok 要求模型真正读到了图片内容。"""
