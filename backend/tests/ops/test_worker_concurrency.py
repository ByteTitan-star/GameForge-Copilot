"""并发消费：_run_one 的 ack/nack 生命周期（at-least-once）+ _consume_once 的 prefetch 并发。
...
（正常退出 ack，抛异常 nack(requeue) 并 re-raise）。重点锁住改造后的不变量：
  ...
  2. TaskLeaseBusy 与普通失败同轨：递增计数 + 退避；超限 → DLQ。
...
async def test_run_one_lease_busy_republishes_with_increment(monkeypatch) -> None:
    """TaskLeaseBusy → 递增计数 + 退避重投（ADR-08，避免原计数无限热循环）。"""
    ...
    await worker_mod._run_one(msg)
    assert msg.acked is True
    assert republished == [3]


async def test_run_one_lease_busy_exhausted_goes_to_dlq(monkeypatch) -> None:
    """TaskLeaseBusy 达到上限 → DLQ，不再 republish。"""
    from app.forge.runner import TaskLeaseBusy
...
async def test_consume_once_sets_prefetch_and_dispatches_concurrently(monkeypatch) -> None:
    """_consume_once：prefetch=max_concurrent_tasks；多条消息的 dispatch 真正并发（peak>1）。

    测 _consume_once 而非 _consume：后者带无限重连 + scheduler，失败路径易挂死会话。
    """
    ...
    async def _slow_dispatch(task: str, payload: dict) -> None:  # noqa: ARG001
        nonlocal in_flight_count, peak
        in_flight_count += 1
        peak = max(peak, in_flight_count)
        if peak >= 3:
            ready.set()
        await release.wait()
        in_flight_count -= 1
    ...
    consume_task = asyncio.create_task(worker_mod._consume_once(in_flight))
    try:
        await asyncio.wait_for(ready.wait(), timeout=2)
        assert channel.prefetch == settings.max_concurrent_tasks
        assert peak == 3
        assert all(m.acked is False for m in messages)
