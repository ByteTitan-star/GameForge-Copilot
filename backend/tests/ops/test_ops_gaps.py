...
async def test_concurrent_run_limit(verified_client: httpx.AsyncClient, monkeypatch) -> None:
    ...
    r1 = await verified_client.post(f"/api/v1/games/{gid}/runs", json={"requirement": "go"})
    assert r1.status_code == 201, r1.text
    r2 = await verified_client.post(f"/api/v1/games/{gid}/runs", json={"requirement": "go2"})
...
async def test_draft_limit(verified_client: httpx.AsyncClient, monkeypatch) -> None:
    ...
    r1 = await verified_client.post("/api/v1/games", json={"title": "a", "requirement": "r"})
    assert r1.status_code == 201
    r2 = await verified_client.post("/api/v1/games", json={"title": "b", "requirement": "r"})
    assert r2.status_code == 429
...
async def test_approve_sends_notification(
    ...
    g = await verified_client.post("/api/v1/games", json={"title": "NotifyMe", "requirement": "r"})
...
def test_transport_reset_noise_downgraded() -> None:
    ...
    # 其他异常 → 不是噪音
    assert not _is_transport_reset_noise({"message": "boom", "exception": RuntimeError()})
