...
async def test_oauth_start_github(client: httpx.AsyncClient, redis_client, _oauth_config) -> None:
...
async def test_oauth_callback_mock_profile(
    ...
    r = await client.get(f"/api/v1/auth/oauth/github/callback?code=fake&state={state}")
    assert r.status_code == 200, r.text
