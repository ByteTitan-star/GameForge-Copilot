...
async def test_rename_draft(verified_client: httpx.AsyncClient) -> None:
    r = await verified_client.post(
        "/api/v1/games", json={"title": "旧名", "requirement": "r"}
    )
...
async def test_pause_cancel_run(
...
async def test_admin_audit_and_games(
    ...
    g = await verified_client.post(
        "/api/v1/games", json={"title": "PubGame", "requirement": "r"}
    )
...
async def test_notifications_inbox(
    ...
    g = await verified_client.post(
        "/api/v1/games", json={"title": "N", "requirement": "r"}
    )
    gid = uuid.UUID(g.json()["data"]["game_id"])
    await store.write_artifact(gid, 1, {"index.html": "<html></html>"})
    async with db.SessionLocal() as s:
    ...
    await admin_client.post(
        f"/api/v1/publish/{sub.json()['data']['publish_request_id']}/approve"
    )
