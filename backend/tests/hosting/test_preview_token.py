...
async def test_mint_and_validate(redis_client) -> None:
    ...
    assert token
    assert await preview_token.validate_preview_token(redis_client, token, game_id=gid, version=2)
...
@pytest.mark.asyncio
async def test_expired_token_invalid(redis_client) -> None:
    gid = uuid.uuid4()
    owner = uuid.uuid4()
    token = await preview_token.mint_preview_token(
        redis_client, game_id=gid, version=1, owner_id=owner
    )
    await redis_client.delete(f"preview:{token}")
    assert not await preview_token.validate_preview_token(
        redis_client, token, game_id=gid, version=1
    )
