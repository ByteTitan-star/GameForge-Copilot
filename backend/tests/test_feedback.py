...
async def test_feedback_non_owner_run_404(
    ...
    # 第二个已验证用户
    await client.post(
        "/api/v1/auth/register", json={"email": "x@b.com", "password": "password123"}
    )
    code = sent["verify:x@b.com"]
    await client.post(
        "/api/v1/auth/verify-email", json={"email": "x@b.com", "code": code}
    )
    r = await client.post(
...
async def test_feedback_requires_auth(client: httpx.AsyncClient) -> None:
    """未认证 → 401。"""
    r = await client.post(
        "/api/v1/me/feedback", json={"run_id": str(uuid.uuid4())}
    )
...
async def test_feedback_admin_email_resolved_from_env(
    ...
    await verified_client.post(
        "/api/v1/me/feedback", json={"run_id": str(run_id), "message": "hi"}
    )
    assert len(notifications) == 1
