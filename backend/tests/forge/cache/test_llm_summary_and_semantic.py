...
async def test_semantic_hard_hit_returns_cached(memory_pinecone) -> None:
    ...
    hit = await semantic_cache_lookup(r, node="entry_router", query=payload, skill_bundle_hash="h1")
...
async def test_semantic_soft_hit_uses_confirm_llm(
    ...
    hit = await semantic_cache_lookup(r, node="entry_router", query=near, skill_bundle_hash="h1")
    assert hit == "greenfield"
...
async def test_semantic_shadow_respects_enabled_flag(
    ...
    ok = await semantic_shadow_record(r, node="entry_router", query={"q": 1}, actual_output="code")
    assert ok is False
    monkeypatch.setattr(settings, "semantic_cache_shadow_enabled", True)
    ok2 = await semantic_shadow_record(r, node="entry_router", query={"q": 2}, actual_output="code")
    assert ok2 is True
