...
async def _login_verified_without_llm(client: httpx.AsyncClient, sent: dict[str, str]) -> None:
...
async def test_create_run_ok_when_default_llm_exists(
    ...
    gid = (await verified_client.post("/api/v1/games", json=GAME_BODY)).json()["data"]["game_id"]
    r = await verified_client.post(f"/api/v1/games/{gid}/runs", json={"requirement": "贪吃蛇"})
