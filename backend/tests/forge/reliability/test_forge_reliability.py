...
async def _make_run(client: httpx.AsyncClient) -> uuid.UUID:
    game = await client.post("/api/v1/games", json={"title": "reliability", "requirement": "test"})
    game_id = game.json()["data"]["game_id"]
    run = await client.post(f"/api/v1/games/{game_id}/runs", json={"requirement": "test"})
...
async def test_outbox_dispatches_committed_run_task(
    ...
    assert any(
        payload.get("run_id") == str(run_id) for _task, payload in MemoryTaskPublisher.captured
    )
    async with db.SessionLocal() as session:
        rows = list(
            (
...
async def test_outbox_publish_failure_remains_retryable(
    ...
    monkeypatch.setattr("app.messaging.outbox.get_task_publisher", lambda: FailingPublisher())
    assert await dispatch_pending() == 0

    async with db.SessionLocal() as session:
        rows = list(
            (
                await session.scalars(select(TaskOutbox).where(TaskOutbox.published_at.is_(None)))
            ).all()
        )
        retryable = [row for row in rows if str(row.payload.get("run_id")) == str(run_id)]
        assert retryable
...
async def test_cancel_closes_unpublished_run_task(
    ...
    async with db.SessionLocal() as session:
        ...
        cancelled = [row for row in rows if str(row.payload.get("run_id")) == str(run_id)]
        assert cancelled
