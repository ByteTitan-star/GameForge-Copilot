...
export function ForgeSidePanel({ tab, onTabChange, items, previewUrl, gameTitle, className }: Props) {
  const t = useT()
  return (
    <section className={cn('flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-white/[0.08] bg-[#12151a]', className)}>
      ...
      {tab === 'log' ? (
        <div className="flex-1 space-y-2 overflow-y-auto p-3 font-mono text-[11px] leading-relaxed">
          {items.length === 0 ? (
            <p className="py-10 text-center text-white/30">{t('timelineEmpty')}</p>
          ) : (
            items.map((it) => (
        ...
        <div className="grid flex-1 place-items-center px-6 text-center">
          <div>
            <p className="text-sm text-white/55">{t('noBuildArtifact')}</p>
            <p className="mt-1 text-xs text-white/30">{t('runCompletePreview')}</p>
          </div>
        </div>
      )}
    </section>
